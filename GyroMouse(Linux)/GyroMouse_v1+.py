...
def run_GUI(): # Function defining Graphical User Interface
    ...
    def read_default_values(): # Reads default values from a text file saved as "default.txt" it contains the data of "values" variable. If the "default.txt" file does not exist data from "values" variable is taken.
        ...
        try: # If "default.txt" file exists
            with open("default.txt","r") as f:
                ...
                values=[values[0].strip()]+[float(values[i]) for i in range(1,len(values))] # Converting data from string to float
                port_name.set(str(values[0]))                                               # Assigning port name
            ...
            values=["/dev/ttyUSB0",1,4.5,2,20,-6,-3,-2,1,-2,1]
            port_name.set(values[0])       # Assigning port name
            ser_num.set(int(values[1]))    # Assigning serial number
    ...
    def calibrate(): # Function to detect errors of gyroscope and accelerometer
        ...
        def get_data(s): # Function to read data from serial port and to find error
            ...
            if max_gz>-9000:
                values[10]=max_gz
                #Button "Calibrate"
                tk.Button(window,text="Calibrate",font=("Helvetica",20,"bold"),fg="Black",activebackground="SlateBlue2",bg="SpringGreen2",bd=0,width=13,command=calibrate).grid(row=5,column=0)
        ...
        try:
            try: # Try to connect to serial device using known serial number of the device
                ...
                for p in ports:
                    if int(p.serial_number)==device_serial_number:
                        device_serial_port=p.device # If serial number matches ( If required serial device is connected ) then save it's port name value
                        break
                s=serial.Serial(device_serial_port,baudrate=115200,timeout=1) # Open the Serial port
                values[0]=device_serial_port # Save port name to "values" variable
                port_name.set(values[0])
                get_data(s)
                
                #Entry  "Port Name"
                # Change port name Entry appearence indicating successfully connected to required serial device
                com_value=tk.Entry(window,width=15,font=("Helvetica", 30, "bold"),fg="SteelBlue4",bg="SpringGreen2",bd=0,textvariable=port_name)
                ...
                tk.Entry(window,width=15,font=("Helvetica",30,"bold"),fg="SteelBlue4",bg="SpringGreen2",bd=3,textvariable=ser_num).grid(row=1,column=2,sticky="W")
                ser_num.set(values[1])
                
            except: # Try to connect to serial device using known COM port of the device
                values[0]=port_name.get()
                device_serial_port=str(values[0])
                s=serial.Serial(device_serial_port,baudrate=115200,timeout=1) # Open the Serial port
                try:
                    for p in ports: # "ports" variable is defined in above try block
                        if p.device==device_serial_port:
                            device_serial_number=int(p.serial_number) # If port name matches ( If required serial device is connected ) then save it's serial number
                            break
                    ...
                    #Entry "Serial number"
                    # Change Serial number Entry appearence indicating invalid serial number
                    tk.Entry(window,width=15,font=("Helvetica",30,"bold"),fg="SteelBlue4",bg="IndianRed2",bd=3,textvariable=ser_num).grid(row=1,column=2,sticky="W")
                    ser_num.set(values[1])
                get_data(s)
                #Entry  "Port Name"
                # Change port name Entry appearence indicating successfully connected to required serial device
            ...
            #Entry  "Port Name"
            # Change port name Entry appearence indicating unable to connect to required serial device
            com_value=tk.Entry(window,width=15,font=("Helvetica", 30, "bold"),fg="SteelBlue4",bg="IndianRed2",bd=0,textvariable=port_name)
            com_value.grid(row=0,column=2)
            port_name.set(values[0])
    ...
    port_name=tk.StringVar() # tkinter variable for port name
    ...
    #Label  "Port Name"
    com=tk.Label(window,text="Enter port name ",font=("Helvetica", 30, "bold"),fg="Blue",bg="white smoke")
    com.grid(row=0,column=0,columnspan=2,sticky="W")

    #Entry  "Port Name"
    com_value=tk.Entry(window,width=15,font=("Helvetica", 30, "bold"),fg="SteelBlue4",bg="white smoke",bd=3,textvariable=port_name)
    com_value.grid(row=0,column=2,sticky="W")
    ...
    #Entry "Serial number"
    tk.Entry(window,width=15,font=("Helvetica",30,"bold"),fg="SteelBlue4",bg="white smoke",bd=3,textvariable=ser_num).grid(row=1,column=2,sticky="W")
    ...
    tk.Entry(window,width=15,font=("Helvetica",30,"bold"),fg="SteelBlue4",bg="white smoke",bd=3,textvariable=mouse_s).grid(row=2,column=2,sticky="W")
    ...
    tk.Entry(window,width=15,font=("Helvetica",30,"bold"),fg="SteelBlue4",bg="white smoke",bd=3,textvariable=scroll_s).grid(row=3,column=2,sticky="W")
    ...
    tk.Entry(window,width=15,font=("Helvetica",30,"bold"),fg="SteelBlue4",bg="white smoke",bd=3,textvariable=vol_s).grid(row=4,column=2,sticky="W")
