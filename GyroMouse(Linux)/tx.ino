...
int left=9,right=7, scroll=5, movement=3;
...
void loop() {
  ...
  Serial.println(-1); // "-1" is the indication of start of data transfer
  recordAccelRegisters();
  recordGyroRegisters();
  printData();
  Serial.println(digitalRead(left));
  Serial.println(digitalRead(right));
  Serial.println(digitalRead(scroll));
  Serial.println(digitalRead(movement));
  Serial.flush(); // This is important else data transfer will stop because of overload of serial register
